...
locals {
  cluster_name_primary  = "my-eks-primary-${random_string.suffix.result}"
  cluster_name_recovery = "my-eks-recovery-${random_string.suffix.result}"
  name                  = "velero-test-${random_string.suffix.result}"
}
...
resource "aws_subnet" "public" {
  ...
  tags = {
    ...
    "kubernetes.io/cluster/${local.cluster_name_recovery}" = "shared"
...
resource "aws_eks_cluster" "recovery_cluster" {
...
resource "aws_eks_node_group" "primary_nodes" {
  ...
  node_group_name = "${aws_eks_cluster.primary_cluster.name}-nodegroup"
...
resource "aws_eks_node_group" "recovery_nodes" {
  cluster_name    = aws_eks_cluster.recovery_cluster.name
  node_group_name = "${aws_eks_cluster.recovery_cluster.name}-nodegroup"
  node_role_arn   = aws_iam_role.eks_nodegroup_role.arn
  subnet_ids      = aws_subnet.public[*].id
  update_config {
    max_unavailable = 1
